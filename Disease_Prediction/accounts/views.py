...
def signup_doctor(request):
    ...
    if request.method == 'POST':
      ...
      if request.POST['username'] and request.POST['email'] and  request.POST['name'] and request.POST['dob'] and request.POST['gender'] and request.POST['address']and request.POST['mobile'] and request.POST['password']and request.POST['password1']   and    request.POST['qualification'] and   request.POST['specialization'] :
          ...
          if password == password1:
              if User.objects.filter(username = username).exists():
                ...
                doctornew = doctor( user=user, name=name, dob=dob, gender=gender, address=address, mobile_no=mobile_no, qualification=qualification, specialization=specialization )
                doctornew.save()
                messages.info(request,'user created sucessfully')
                print("doctorcreated")
